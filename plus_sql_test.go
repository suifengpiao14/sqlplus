...
func TestParse(t *testing.T) {
	...
	scenes := sqlplus.Scenes{sqlplus.Scene_Select_Where}
	t.Run("select with where", func(t *testing.T) {
		sqlStr = "select * from user where id=1 or id=2;"
		newSqlStr, err = sqlplus.WithPlusScene(sqlStr, scenes, tableColumn)
	...
	t.Run("insert", func(t *testing.T) {
		...
		scenes := sqlplus.Scenes{sqlplus.Scene_Insert_Column}
		newSqlStr, err = sqlplus.WithPlusScene(sqlStr, scenes, tableColumn)
		require.NoError(t, err)
		fmt.Println(newSqlStr)
	})
